...
//  ImGuiPlatformIO interface. 2025-03-10: When dealing with OEM keys, use
//  scancodes instead of translated keycodes to choose ImGuiKey values. (#7136,
//  #7201, #7206, #7306, #7670, #7672, #8468) 2025-02-21: [Docking]
//  WM_SETTINGCHANGE's SPI_SETWORKAREA message also triggers a refresh of
//  monitor list. (#8415) 2025-02-18: Added ImGuiMouseCursor_Wait and
//  ImGuiMouseCursor_Progress mouse cursor support. 2024-11-21: [Docking] Fixed
//  a crash when multiple processes are running with multi-viewports, caused by
//  misusage of GetProp(). (#8162, #8069) 2024-10-28: [Docking] Rely on property
//  stored inside HWND to retrieve context/viewport, should facilitate attempt
//  to use this for parallel contexts. (#8069) 2024-09-16: [Docking] Inputs:
//  fixed an issue where a viewport destroyed while clicking would hog mouse
//  tracking and temporary lead to incorrect update of HoveredWindow. (#7971)
...
static bool ImGui_ImplWin32_InitEx(void* hwnd, bool platform_has_own_dc) {
  ...
  // So as we store a pointer in IMGUI_CONTEXT we need to make sure we only call
  // GetPropA() on windows owned by our process.
...
static ImGuiViewport* ImGui_ImplWin32_FindViewportByPlatformHandle(
  ...
  // We cannot use ImGui::FindViewportByPlatformHandle() because it doesn't take
  // a context. When called from ImGui_ImplWin32_WndProcHandler_PlatformWindow()
  // we don't assume that context is bound.
...
static void ImGui_ImplWin32_CreateWindow(ImGuiViewport* viewport) {
  ...
  ImGui_ImplWin32_GetWin32StyleFromViewportFlags(viewport->Flags, &vd->DwStyle,
                                                 &vd->DwExStyle);
  vd->HwndParent =
      ImGui_ImplWin32_GetHwndFromViewportID(viewport->ParentViewportId);
...
static void ImGui_ImplWin32_SetWindowTitle(ImGuiViewport* viewport,
  ...
  // Calling SetWindowTextW() in a project where UNICODE is not set doesn't work
  // but there's a trick which is to pass it directly to the DefWindowProcW()
  // handler. See:
...
static LRESULT CALLBACK ImGui_ImplWin32_WndProcHandler_PlatformWindow(
  ...
  if (ImGui_ImplWin32_WndProcHandlerEx(hWnd, msg, wParam, lParam, io))
    ...
    switch (msg) {
      ...
      case WM_NCHITTEST:
        ...
        // call io.AddMouseViewportEvent() correctly. (which is optional). The
  ...
  if (result == 0) result = DefWindowProc(hWnd, msg, wParam, lParam);
  return result;
}
