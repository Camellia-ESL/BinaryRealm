...
enum ImGuiConfigFlags_ {
  ...
  ImGuiConfigFlags_DpiEnableScaleViewports =
      1 << 14,  // [BETA: Don't use] FIXME-DPI: Reposition and resize imgui
                ...
                // useful for the main viewport hosting other window). Note that
...
struct ImGuiStyle {
  ...
  bool GlassBlurBackground;  // Enable glassy blur effect for the background of
                             // the windows created with viewports.
...
struct ImGuiIO {
  ...
  ClearInputCharacters();  // [Obsoleted in 1.89.8] Clear the current frame text
                           // input buffer. Now included within
                           // ClearInputKeys().
#endif
  ...
  ImGuiID
      MouseHoveredViewport;  // (Optional) Modify using
                             ...
                             // io.BackendFlags |=
                             // ImGuiBackendFlags_HasMouseHoveredViewport if you
                             // can provide this info. If you don't imgui will
                             // infer the value using the rectangles and last
                             // focused time of the viewports it knows about
                             // (ignoring other OS windows).
  ...
  ImGuiKeyData KeysData[ImGuiKey_NamedKey_COUNT];  // Key state for all known
                                                   // keys. Use IsKeyXXX()
...
struct ImGuiWindowClass {
  ...
  ImGuiDockNodeFlags
      DockNodeFlagsOverrideSet;  // [EXPERIMENTAL] Dock node flags to set when a
                                 // window of this class is hosted by a dock
...
struct ImFontAtlasCustomRect {
  ...
  unsigned int
      GlyphID : 31;  // Input    // For custom font glyphs only (ID < 0x110000)
...
struct ImFont {
  ...
  ImFontConfig*
      Sources;  // 4-8   // in  // Pointer within ContainerAtlas->Sources[], to
...
enum ImGuiViewportFlags_ {
  ...
  ImGuiViewportFlags_CanHostOtherWindows =
      1 << 11,  // Viewport can host multiple imgui windows (secondary viewports
                ...
                // there's still probably code making the assumption that this
                // is always and only on the MainViewport. Will fix once we add
...
struct ImGuiViewport {
  ...
  void* PlatformHandleRaw;  // void* to hold lower-level, platform-native window
                            // handle (always HWND on Win32 platform, unused for
                            // other platforms).
...
struct ImGuiPlatformIO {
  ...
  // backends will often provide both Platform and Renderer interfaces together
  // and so may not need to use all functions. Platform functions are typically
  // called _before_ their Renderer counterpart, apart from Destroy which are
