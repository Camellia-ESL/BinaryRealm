...
bool ImGui::ToggleButton(const char* label, bool* v) {
  ...
  const ImRect total_bb(
      pos, pos + ImVec2(width + (label_size.x > 0.0f
                                     ? style.ItemInnerSpacing.x + label_size.x
                                     : 0.0f),
                        label_size.y + style.FramePadding.y * 2.0f));
...
// above.
void ImGui::ImageWithBg(ImTextureID user_texture_id, const ImVec2& image_size,
  ...
  if (g.Style.ImageBorderSize > 0.0f)
    window->DrawList->AddRect(bb.Min, bb.Max, GetColorU32(ImGuiCol_Border),
                              0.0f, ImDrawFlags_None, g.Style.ImageBorderSize);
  if (bg_col.w > 0.0f)
...
void ImGui::Image(ImTextureID user_texture_id, const ImVec2& image_size,
  ...
  ImageWithBg(user_texture_id, image_size, uv0, uv1, ImVec4(0, 0, 0, 0),
              tint_col);
  PopStyleColor();
  PopStyleVar();
}
...
bool ImGui::BeginMenuBar() {
  ...
  const float border_top = ImMax(
      IM_ROUND(window->WindowBorderSize * 0.5f - window->TitleBarHeight), 0.0f);
...
void ImGui::TabBarAddTab(ImGuiTabBar* tab_bar, ImGuiTabItemFlags tab_flags,
  ...
  new_tab.LastFrameVisible =
      tab_bar->CurrFrameVisible;  // Required so BeginTabBar() doesn't ditch the
                                  // tab
